...
| 기술 | 역할 | 선택 이유 |
|------|------|-----------|
| **FastAPI + asyncio** | 메인 서버 | 이 시스템의 작업 대부분이 IO 대기다 — KIS API 응답 기다리기, WebSocket 틱 기다리기, Redis 응답 기다리기. 모든 핵심 함수를 `async def` 코루틴으로 작성하고 `asyncio.create_task()`로 이벤트 루프에 등록했다. 코루틴은 `await` 지점에서 실행을 멈추고 제어권을 이벤트 루프에 넘기기 때문에, 단일 스레드에서 WebSocket 수신·HTTP 처리·조건 평가가 IO 대기 시간을 겹쳐 동시에 돌아간다. 동기 프레임워크(Flask 등)는 하나를 처리하는 동안 나머지가 멈춤 |
| **Redis** | 가격 캐시 / 감시 조건 / 지표 캐시 | 서버 재시작 시 메모리 dict는 날아가지만 Redis는 유지된다. 감시 조건·지표 캐시가 재시작 후에도 살아있음. 지표 캐시에 TTL 30분을 설정해 갱신 실패 시 자동 만료 처리 |
| **asyncio.Queue** | 이벤트 라우팅 | watch 트리거 이벤트는 하루 수십 건, 영속성 불필요. Redis Pub/Sub·RabbitMQ 같은 외부 브로커는 오버엔지니어링 |
| **simpleeval** | watch 수식 평가 | AI가 작성한 수식(`"rsi < 30 and volume > avg_volume * 1.5"`)을 런타임에 실행해야 한다. `eval()`을 쓰면 LLM이 `__import__('os').system('rm -rf /')` 같은 수식을 만들어낼 경우 그대로 실행된다. simpleeval은 사칙연산·비교·논리 연산자만 허용하고 함수 호출·import를 차단해 수식 실행 범위를 숫자 계산으로만 제한 |
| **pandas_ta** | 기술 지표 계산 | RSI·볼린저·스토캐스틱을 직접 구현하면 수식 실수·엣지케이스 버그 위험이 크다. 검증된 라이브러리를 사용. 단, `ta.bbands()` 결과 컬럼명이 `BBL`이 아니라 `BBL_20_2.0`처럼 파라미터가 붙어서 생성돼 `df["BBL"]`로 접근하면 KeyError가 난다. `next(c for c in bb.columns if c.startswith("BBL_"))` 패턴으로 동적 탐색해야 함 |
| **Claude / Gemini / Groq** | LLM 추론 | Provider 추상화로 config 1줄 전환. 특정 공급자 장애 시 즉시 대체 가능 |
| **Electron** | 데스크탑 앱 | 브라우저 대신 네이티브 데스크탑 앱으로 구현 |
