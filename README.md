...
시스템은 크게 세 개의 독립적인 루프로 구성된다.

```
┌─────────────────────────────────────────────────────────────┐
│  [루프 1] 사용자 대화                                        │
│                                                             │
│  Jarvis 앱 ──자연어──▶ FastAPI ──▶ AI Agent                │
│                                        │                    │
│                              Tool-Use 반복 루프             │
│                                        │                    │
│                         ┌──────────────┼──────────────┐    │
│                         ▼              ▼              ▼    │
│                      KIS REST       Redis          PostgreSQL│
│                    (현재가·주문)   (가격·감시·지표)  (이력)  │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│  [루프 2] 실시간 가격 수신 (상시 실행)                       │
│                                                             │
│  KIS WebSocket ──틱──▶ MarketDataCollector ──▶ Redis        │
│  (체결가 스트리밍)                           price:{code}   │
└─────────────────────────────────────────────────────────────┘
...
│  [루프 3] 감시 조건 평가 (상시 실행)                         │
│                                                             │
│  IndicatorCache ──5분──▶ Redis          (지표 갱신)         │
│                        ai:indicators:{code}                 │
│                                                             │
│  EventDetector ──10초──▶ Redis 읽기                         │
│                          price + indicators                 │
│                              │ 조건 충족                    │
│                              ▼                              │
│                       AI Agent.handle_event()               │
│                       (자율 매매 판단)                       │
└─────────────────────────────────────────────────────────────┘
```

루프 1은 사용자 요청에만 반응한다.  
루프 2·3은 서버가 켜져 있는 동안 항상 돌아간다. 사용자가 자리를 비워도 조건이 충족되면 AI가 스스로 판단한다.
...
| 기술 | 역할 | 선택 이유 |
|------|------|-----------|
| **FastAPI + asyncio** | 메인 서버 | KIS WebSocket 수신, 사용자 HTTP 요청, 자동매매 루프가 동시에 돌아야 함. 동기 프레임워크(Flask 등)는 WebSocket과 병렬 처리 불가 |
